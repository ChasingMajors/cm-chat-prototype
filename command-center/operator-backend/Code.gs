...
function runScheduledChecklistPendingValidationAction_(action, key) {
  ...
  return {
    ok: true,
    validated: validated,
    status: validated ? "pending_visual_validation" : attempts >= CM_PUBLIC_VALIDATION_RETRY_LIMIT ? "failed" : "pending_public_validation",
    sport: sport,
    code: code,
    ...
    validationResult: validated
      ...
      : attempts >= CM_PUBLIC_VALIDATION_RETRY_LIMIT
      ? "Agent attempted public checklist JSON validation " + attempts + " times and failed. Create a repair task for publish/index validation. Expected "
...
function appendPublicValidationFixTask_(memory, action, result, now) {
  ...
  const sport = safeString_(action && action.sport).trim();
  const code = safeString_(action && action.code).trim();
  const attempts = Number(action && action.pendingValidationAttempts || result && result.pendingValidationAttempts || 0);
